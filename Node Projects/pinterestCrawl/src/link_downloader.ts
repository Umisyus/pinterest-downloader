...
// import { crawl } from './pin_ex.js';
import * as fs from 'fs'

export async function link_downloader() {
    playwright.chromium.launch({ headless: false, devtools: true }).then(async (browser) => {
        // Pinterest Cookies
        ...
        let logg = await page.evaluate(hello => {
            // @ts-ignore
            const $x = xp => {
            ...
            function get_boards(boards_selector = "div[data-test-id='pwt-grid-item']") {
                ...
                // @ts-ignore
                let links = (i => i.querySelector('a')).map(i => i.href)
                console.log(links);
