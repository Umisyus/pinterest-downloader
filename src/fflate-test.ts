...
export async function zipKVS(
  ...
  let isAtHome = !Actor.isAtHome(); /* TEST */
  ...
  for await (const records of f) {
    ...
    await zip(zipObj as AsyncZippable, { level: 0 }).then(async (res) => {
      ...
      log.info(`FILE ${zip_file_name} SIZE: ${res.length} BYTES`);
...
async function* IteratorGetKVSValues(
  KVS_ID: string,
  API_TOKEN?: string | undefined,
  FILES_PER_ZIP: number = 1000,
  MAX_ZIP_SIZE_MB: number = 200
) {
  let client = new ApifyClient({ token: API_TOKEN });

  let kvs = (await client.keyValueStores().list()).items.find(
    (k) => k.id === KVS_ID || k.name === KVS_ID || k.title === KVS_ID
  );
  ...
  do {
    ...
    for await (const i of images) {
      ...
      const isLimitReached =
        FILES_PER_ZIP && FILES_PER_ZIP > 0 && items.length >= FILES_PER_ZIP;
