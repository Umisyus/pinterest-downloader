...
async function zipToKVS(client: ApifyClient) {
    ...
    async function writeManyZips() {
        ...
        for (const kvs of kvs_items.items) {
            let kvs_item = await (Actor.openKeyValueStore(kvs.name ?? kvs.title ?? kvs.id));
            ...
            let ff = await archiveKVS(kvs_item);
            let nom = `${randomUUID()}_${kvs.name ?? kvs.title ?? kvs.id}`
            saveToKVS(ff, nom);
...
async function archiveKVS(store: KeyValueStore) {
    ...
    await new Promise<void>(async (resolve, reject) => {
        ...
        await store.forEachKey(async (key) => {
            const val = await store.getValue(key) as any
            archive.append(val, { name: `${key}` })
        })
